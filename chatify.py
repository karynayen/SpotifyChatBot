...
@dataclasses.dataclass
class AddArtistsResponse(AgentResponse):
    """
    The agent added the following artists to the seed artists.
    """
...
Don't add tracks to a playlist multiple times.
"""
...
"""
    ...
    def recommend_songs(self, seed_artists=None, seed_tracks=None, seed_genres=None, valence=None, energy=None, danceability=None, limit=5):
        ...
        if seed_genres:
            recommendations = recommendations[recommendations['track_genre'].isin(seed_genres)]
        else:
    ...
    def say(self, user_message: str) -> AgentResponse:
        ...
        if "get_recommendations" in code_text:
            ...
            return GetRecommendationsResponse(text=user_recc_resp, recommendations=res)
        if "add_to_seed_artists" in code_text:
            ...
            return AddArtistsResponse(text=message, artist_seeds=self.seed_artists)
        if "add_to_seed_tracks" in code_text:
            ...
            return AddTracksResponse(text=message, track_seeds=self.seed_tracks)
        if "add_to_seed_genres" in code_text:
            new_genre = self.seed_genres[-1]
            message = f"Added {new_genre} to seed genres."
            return AddGenresResponse(text=message, genre_seeds=self.seed_genres)
        if "modify_mood" in code_text:
            ...
            return ModifyMoodResponse(text=message, valence=self.target_valence, energy=self.target_energy, danceability=self.target_danceability)
        if "add_tracks_to_playlist" in code_text:
            message = "Added tracks to playlist.\n" # some how also add a spotify iframe... 
            return AddToPlaylistResponse(text=message, tracks=res)
